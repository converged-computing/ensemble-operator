...
# flux-framework.org/ensemble-operator-bundle:$VERSION and flux-framework.org/ensemble-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= ghcr.io/converged-computing/ensemble-operator
...
IMG ?= ghcr.io/converged-computing/ensemble-operator

# Testing image (for development mostly)
DEVIMG ?= ghcr.io/converged-computing/ensemble-operator:test
ARMIMG ?= ghcr.io/converged-computing/ensemble-operator:arm
...
build-config: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default > examples/dist/ensemble-operator.yaml
...
build-config-arm: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	...
	$(KUSTOMIZE) build config/default > examples/dist/ensemble-operator-arm.yaml
...
test-deploy: manifests kustomize
	...
	$(KUSTOMIZE) build config/default > examples/dist/ensemble-operator-dev.yaml

.PHONY: test-deploy-recreate
test-deploy-recreate: test-deploy
	kubectl delete -f ./examples/dist/ensemble-operator-dev.yaml || echo "Already deleted"
	kubectl apply -f ./examples/dist/ensemble-operator-dev.yaml
